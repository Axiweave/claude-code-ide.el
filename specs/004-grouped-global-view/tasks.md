...
- [X] T036 Complete the integrated UI/backend/persistence acceptance matrix in `specs/004-grouped-global-view/quickstart.md`. Use compatible `emacsclient` reloads or a fresh test instance for changed structs. Verify supported local backends, the existing explicit remote-backend limitation, real keyboard behavior, view restoration, and request-free ordinary interactions.
