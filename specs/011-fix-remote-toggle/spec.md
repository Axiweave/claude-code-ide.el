...
A user attaches an existing Agent on a remote host. The user invokes the standard session toggle from that Agent Session or its managed remote project view. The command hides or restores the same attached Session, just as it does for a local Session.
...
**Independent Test**: Attach a remote Agent whose project directory text also exists locally. Invoke the toggle from its terminal and managed project view. Confirm that each invocation affects only that remote Session.
...
- **Remote Context**: The active remote terminal or managed project view that identifies the intended Session and host.
