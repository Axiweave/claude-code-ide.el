  ...
  (claude-code-ide-tests--with-agent-state-fixture buffer
                                                   ...
                                                   (dolist (state '("done" "failed"))
                                                     ...
                                                     (should (eq (buffer-local-value 'claude-code-ide-session-agent-state buffer)
                                                                 'idle)))
                                                   ;; Reconnect: the new owner replays the acknowledged state.
                                                   (claude-code-ide-mcp-sse--forget-session "sid")
                                                   (should-not (buffer-local-value 'claude-code-ide-session-agent-state buffer))
                                                   (puthash "new" (list :process nil :root nil) claude-code-ide-mcp-sse--sessions)
                                                   (claude-code-ide-tests--dispatch-agent-state "new" "failed")
                                                   (should (eq (buffer-local-value 'claude-code-ide-session-agent-state buffer)
                                                               'idle))
                                                   (claude-code-ide-tests--dispatch-agent-state "new" "working")
  ...
  (let* ((directory "/tmp/agent-state/project/")
    ...
    (unwind-protect
        (progn
          ...
          (dolist (case '((needs-input . claude-code-ide-manager-attention-session-face)
                          (failed . claude-code-ide-manager-attention-session-face)
                          (done . claude-code-ide-manager-done-session-face)
                          (working . claude-code-ide-manager-working-session-face)
                          (idle . nil)))
            (with-current-buffer session-buffer
              (setq-local claude-code-ide-session-agent-state (car case)))
            (should (eq (claude-code-ide-manager--row-face '(:type global) directory)
                        (cdr case)))))
      (ignore-errors (delete-process process))
...
(ert-deftest claude-code-ide-test-zmx-infer-cli-command ()
  ...
  (let ((claude-code-ide-cli-path "/opt/custom/mycli"))
    ...
    (should (equal (claude-code-ide-zmx-infer-cli-command "command omp") "omp"))
    (should (equal (claude-code-ide-zmx-infer-cli-command "exec env FOO=bar omp --continue")
                   "omp"))
    (should-not (claude-code-ide-zmx-infer-cli-command "command"))
...
(ert-deftest claude-code-ide-test-zmx-attach-reveals-via-manager-layout ()
  "Adoption suppresses initial display and reveals via the manager switch."
  (let (suppressed switched)
    (cl-letf (((symbol-function 'claude-code-ide-zmx--ensure) #'ignore)
              ((symbol-function 'claude-code-ide-zmx-list-sessions)
               (lambda () '((:name "demo" :start_dir "/tmp/proj" :cmd "omp"))))
              ((symbol-function 'completing-read)
              ...
              ((symbol-function 'claude-code-ide--create-session)
               (lambda (&rest _)
                 (setq suppressed claude-code-ide--suppress-initial-display)
                 (claude-code-ide-session-create :id "adopted-id"
                                                 :directory "/tmp/proj/")))
              ((symbol-function 'claude-code-ide-manager-switch-to-session)
               (lambda (key &rest _) (setq switched key))))
      (claude-code-ide-attach))
    (should suppressed)
    (should (equal switched "adopted-id"))))

(ert-deftest claude-code-ide-test-zmx-attach-candidate-format ()
  "Candidates show project basename and title; the zmx name is invisible."
  (let (candidate)
    (cl-letf (((symbol-function 'claude-code-ide-zmx--ensure) #'ignore)
              ((symbol-function 'claude-code-ide-zmx-list-sessions)
               (lambda () '((:name "cci-omp-proj-Yj3i8Q" :start_dir "/tmp/proj"
                                   :cmd "omp" :title "Fix_parser_bug"))))
              ((symbol-function 'completing-read)
               (lambda (_prompt candidates &rest _)
                 (setq candidate (caar candidates))))
              ((symbol-function 'claude-code-ide--create-session) #'ignore))
      (claude-code-ide-attach))
    (should (equal candidate
                   "proj  Fix parser bug  omp  cci-omp-proj-Yj3i8Q"))
    (let ((name-start (- (length candidate) (length "  cci-omp-proj-Yj3i8Q"))))
      (should (get-text-property name-start 'invisible candidate))
      (should-not (get-text-property 0 'invisible candidate)))))

(ert-deftest claude-code-ide-test-zmx-attach-excludes-live-sessions ()
  "Sessions already attached in this Emacs never appear in the picker."
  (let (candidates-seen created)
    (cl-letf (((symbol-function 'claude-code-ide-zmx--ensure) #'ignore)
              ((symbol-function 'claude-code-ide--zmx-live-names)
               (lambda () '("cci-omp-here-attached")))
              ((symbol-function 'claude-code-ide-zmx-list-sessions)
               (lambda () '((:name "cci-omp-here-attached" :start_dir "/tmp/a" :cmd "omp")
                            (:name "cci-omp-other" :start_dir "/tmp/b" :cmd "omp"))))
              ((symbol-function 'completing-read)
               (lambda (_prompt candidates &rest _)
                 (setq candidates-seen (mapcar #'car candidates))
                 (caar candidates)))
              ((symbol-function 'claude-code-ide--create-session)
               (lambda (&rest args) (setq created args) nil)))
      (claude-code-ide-attach))
    (should (equal (length candidates-seen) 1))
    (should (string-match-p "cci-omp-other" (car candidates-seen)))
    (should (equal (nth 3 created) "cci-omp-other"))))

(ert-deftest claude-code-ide-test-zmx-attach-all-attached-logs ()
  "Attach logs instead of prompting when every session is attached."
  (let (prompted)
    (cl-letf (((symbol-function 'claude-code-ide-zmx--ensure) #'ignore)
              ((symbol-function 'claude-code-ide--zmx-live-names)
               (lambda () '("cci-omp-here-attached")))
...
(ert-deftest claude-code-ide-test-zmx-set-title-encodes-charset ()
  "Titles are sanitized to zmx's [a-zA-Z0-9-_.] label charset."
  (let (captured)
    (cl-letf (((symbol-function 'start-process)
      ...
      (claude-code-ide-zmx-set-title "cci-omp-proj-x" "π ⠧ Speckit: fill v1.0")
      (should (equal (car (last captured)) "title=Speckit_fill_v1.0"))
      (setq captured nil)
      (claude-code-ide-zmx-set-title "cci-omp-proj-x" "   ")
      (should-not captured)
      (claude-code-ide-zmx-set-title "cci-omp-proj-x" "π ⠧")
      (should-not captured)
