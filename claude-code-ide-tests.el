...
(ert-deftest claude-code-ide-test-create-session-reattach-reveals-via-manager ()
  "Reattaching to a zmx session reveals it via the manager, not the side window."
  (let ((claude-code-ide--sessions (make-hash-table :test #'equal))
        (displayed nil)
        (switched nil)
        (buffer (generate-new-buffer "*claude-code-reattach-reveal-test*"))
        (process (start-process "mock-claude-reattach-reveal" nil "true")))
    (unwind-protect
        (cl-letf (((symbol-function 'claude-code-ide--terminal-ensure-backend)
                   (lambda () nil))
                  ((symbol-function 'claude-code-ide--zmx-launch-spec)
                   (lambda (&rest _) '("cci-omp-proj-old1" . t)))
                  ((symbol-function 'claude-code-ide-mcp-start)
                   (lambda (&rest _) 12345))
                  ((symbol-function 'claude-code-ide--create-terminal-session)
                   (lambda (&rest _args) (cons buffer process)))
                  ((symbol-function 'claude-code-ide-mcp-server-session-started)
                   (lambda (&rest _args) nil))
                  ((symbol-function 'set-process-sentinel)
                   (lambda (&rest _args) nil))
                  ((symbol-function 'sleep-for)
                   (lambda (&rest _args) nil))
                  ((symbol-function 'claude-code-ide--display-buffer-in-side-window)
                   (lambda (_buffer) (setq displayed t)))
                  ((symbol-function 'claude-code-ide-manager-switch-to-session)
                   (lambda (key &rest _) (setq switched key)))
                  ((symbol-function 'claude-code-ide-log)
                   (lambda (&rest _args) nil)))
          (let ((session (claude-code-ide--create-session "/tmp/proj/" t nil)))
            (should-not displayed)
            (should (equal switched (claude-code-ide-session-id session)))))
      (when (buffer-live-p buffer)
        (kill-buffer buffer))
      (when (process-live-p process)
        (delete-process process)))))
