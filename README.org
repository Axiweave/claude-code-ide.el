...
The =eat= backend is a pure Elisp terminal emulator that may work better in some environments where =vterm= compilation is problematic. The =ghostel= backend currently has basic integration only: session creation, core input dispatch, idle/working-state tracking, and backend selection work, but the vterm/eat-specific flicker and reflow workarounds are intentionally not enabled there yet.
