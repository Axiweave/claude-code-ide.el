...
Remote attachment supports the Ghostel backend only. Future versions will deprecate vterm and Eat.
...
Remote Project views support Magit or Dired. Use their normal commands for interactive Git and file access.
...
One exact host and Git Worktree share one Project-view buffer.
Non-Git directories use their exact directory identity.
...
Press manager =R= to reset the full layout, clear suppression, and run a fresh health check.
...
| Key | Command | Effect |
|-----+---------+--------|
| =o= | =claude-code-ide-manager-open-remote= | Choose a Configured host and repository, select a Worktree, then reuse/attach/start its Agent. A prefix argument starts an explicit sibling Agent. |
...
=claude-code-ide-remote-launch-config= is an alist keyed by exact Configured host. Each value is a property list with optional =:executable= and =:args=:
...
The default executable is the globally selected Agent's standard command name, resolved on that host's =PATH=. Remote arguments default to an empty list. Local executable paths, local CLI flags, and local MCP settings are never inherited by a remote launch.

Backend commands and Agents keep the host's locale and umask.
The runner uses private permissions and fixed protocol parsing only for its own work.
...
In grouped global view, =E= includes live and disconnected rows under fixed project headings.
...
Apply rejects edited labels or headings, missing rows, and changed project membership.
...
Repeated unchanged reports and output-only changes do not interrupt the pass.
...
This policy does not change Pi, Eat, vterm, standalone omp, or the parent Emacs environment.
