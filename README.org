...
Reattachment works with stock zmx. Emacs runs =zmx attach <name> false=: an existing session is adopted, and a vanished one yields a short-lived session that runs =false= and exits, so no shell or agent starts by accident. This applies to local adoption and remote reattachment. Ordinary local session creation stays unchanged. See the [[file:specs/003-attach-remote-agents/contracts/zmx-attach-guard.md][attach guard contract]] for the verified versions.
...
Press =D= or =X= to detach the local client. Press =K= to Stop the exact selected target. Stop asks for confirmation and names the host, target, and effect on all clients. Emacs removes the row only after zmx confirms the kill and a separate request confirms absence. A timeout or ambiguous result leaves the row intact. Emacs never retries a kill.
...
- =D= or =X= detaches the selected zmx-backed session from Emacs and keeps its agent process running
