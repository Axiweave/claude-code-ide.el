...
With =lane=, a repository without a =.lane/= store gets one question before =lane init=, because that command writes =AGENTS.md=. Set =claude-code-ide-lane-init-protocol= to =nil= to skip both: the bare =.lane/= directory is created silently. Every lane command works on a bare store, including =note= and =why=. Only the =AGENTS.md= protocol is lost, which is what tells agents to use lane memory, so run =lane init= by hand if you want that later. Names that already exist as a branch, or as a lane, are refused before anything is created.
