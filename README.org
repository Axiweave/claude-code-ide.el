...
- Output-driven idle and working indicators in the [[*CC Manager][CC Manager]]
...
- Killing the buffer or closing the window detaches only. =M-x claude-code-ide-stop= asks before it kills the zmx session, because that stops the agent for every attached client. From a terminal, use =zmx attach <name>= and =zmx kill <name>=.

Known limitation: environment variables such as the Claude Code MCP port are frozen when the zmx session is created. If you reattach from a different Emacs instance, or after an Emacs restart, port-based integrations (MCP, SSE) break for that session. Plain terminal use of the agent is unaffected.
...
- =X= detaches the selected zmx-backed session from Emacs and keeps its agent process running
