...
With =lane=, a repository without a =.lane/= store gets one question before =lane init=, because that command writes =AGENTS.md=. Set =claude-code-ide-lane-init-protocol= to =nil= to skip both: the bare =.lane/= directory is created silently, and lane's memory and note system stays unused. Names that already exist as a branch, or as a lane, are refused before anything is created.
