...
| =M-x claude-code-ide-manager-toggle-sidebar= | Toggle the CC Manager sidebar                   |
| =M-x claude-code-ide-manager-focus=       | Focus the CC Manager sidebar                      |
...
- Open the main transient and use:
  - =t= to toggle the manager sidebar
...
- The row shows the project basename
- The full project path is available via hover text
- The currently active session row is highlighted
- Pinned rows are marked with = [P] =
...
Each row reserves a left gutter so the session names stay aligned whether or not an idle marker is present.

- Idle-enabled rows that are currently idle show =🔔= in that gutter
- Rows that are not idle keep the same alignment with blank gutter padding
...
- =claude-code-ide-session-idle-delay= controls the idle timeout before a session is considered idle
...
(setq claude-code-ide-session-idle-default-enabled t
      claude-code-ide-session-idle-delay 30)
