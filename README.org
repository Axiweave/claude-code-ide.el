...
- Press =?= in the sidebar to open its manager menu.
  ...
  - =w= creates a worktree and starts the default Agent in it. It asks for one name that becomes the branch and the tree. =C-u w= creates the tree and opens =magit-status= without an Agent.
...
With =lane=, a repository without a =.lane/= store gets one question before =lane init=, because that command writes =AGENTS.md=. Names that already exist as a branch, or as a lane, are refused before anything is created.
